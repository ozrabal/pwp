...
class Walker_Bootstrapnavwalker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if (strcasecmp($item->title, 'divider') == 0) {
			...
			} else if(isset($args->has_children) && $args->has_children && $depth === 0) {
...
//dump($item);
			...
			$disabled = null;
			if($item->menu_item_parent != 0){ $disabled = 'disabled';}
			$attributes .= (isset($args->has_children) && $args->has_children) 	    ? ' data-toggle="dropdown" data-target="#" class="dropdown-toggle '.$disabled.'"' : '';
